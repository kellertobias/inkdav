    ...
    companion object {
        ...
        val MIGRATION_1_2 = object : Migration(1, 2) {
            override fun migrate(db: SupportSQLiteDatabase) {
                ...
                db.execSQL("CREATE INDEX IF NOT EXISTS index_calendar_occurrences_startEpochMillis ON calendar_occurrences(startEpochMillis)")
                db.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS index_calendar_occurrences_collectionId_uid_originalStartEpochMillis ON calendar_occurrences(collectionId, uid, originalStartEpochMillis)")
                db.execSQL("CREATE TABLE IF NOT EXISTS mirror_bindings (id TEXT NOT NULL PRIMARY KEY, collectionId TEXT NOT NULL, remoteRootHref TEXT NOT NULL, localTreeUri TEXT NOT NULL, displayName TEXT NOT NULL, enabled INTEGER NOT NULL, state TEXT NOT NULL, lastCompleteSyncAt INTEGER, lastError TEXT, itemLimit INTEGER NOT NULL)")
                db.execSQL("CREATE INDEX IF NOT EXISTS index_mirror_bindings_collectionId ON mirror_bindings(collectionId)")
                db.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS index_mirror_bindings_collectionId_remoteRootHref ON mirror_bindings(collectionId, remoteRootHref)")
                ...
                db.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS index_mirror_entries_bindingId_relativePath ON mirror_entries(bindingId, relativePath)")
                db.execSQL("CREATE TABLE IF NOT EXISTS task_widget_configs (appWidgetId INTEGER NOT NULL PRIMARY KEY, mode TEXT NOT NULL, listCollectionId TEXT, lookAheadDays INTEGER NOT NULL)")
                db.execSQL("CREATE TABLE IF NOT EXISTS task_widget_exclusions (appWidgetId INTEGER NOT NULL, collectionId TEXT NOT NULL, PRIMARY KEY(appWidgetId, collectionId))")
