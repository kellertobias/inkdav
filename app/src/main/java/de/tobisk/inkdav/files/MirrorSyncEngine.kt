...
class MirrorSyncEngine(private val context: Context, private val dao: InkDavDao, private val dav: DavClient) {
    ...
    suspend fun synchronize(account: DavAccountEntity, password: CharArray, collection: DavCollectionEntity, binding: MirrorBindingEntity) {
        ...
        try {
            ...
            if (remote.isEmpty() && existing.isNotEmpty()) error("Remote mirror scan unexpectedly returned no entries; deletion reconciliation aborted")
            ...
            (remote.keys + local.keys + existing.keys).sortedBy { it.count { ch -> ch == '/' } }.forEach { path ->
                ...
                if (r?.value?.isCollection == true || l?.document?.isDirectory == true) {
                    if (r == null && l != null && old == null) { dav.makeCollection(account, password, remoteHref(binding.remoteRootHref, path)); return@forEach }
                ...
                val contentEqual = if (r != null && l != null && (old == null || (remoteFingerprint != old.baselineRemoteEtag && localFingerprint != old.baselineLocalHash))) remoteHash(account, password, r.value.href) == localFingerprint else false
                when (MirrorReconciler.decide(binding.state == MirrorState.BASELINED, old?.baselineRemoteEtag, old?.baselineLocalHash, remoteFingerprint, localFingerprint, contentEqual)) {
                    ...
                    MirrorDecision.UPLOAD -> if (l != null) {
                        ...
                        val uploadedEtag = dav.putStream(account, password, href, { requireNotNull(context.contentResolver.openInputStream(l.document.uri)) }, l.document.length().takeIf { it >= 0 }, l.document.type ?: "application/octet-stream", old?.baselineRemoteEtag, createOnly = old == null)
                        remoteFingerprint = uploadedEtag ?: remoteFingerprint
                        effectiveRemote = Remote(path, DavResource(href = href, displayName = l.document.name.orEmpty(), etag = uploadedEtag, contentType = l.document.type, size = l.document.length()))
                    }
                    ...
                    MirrorDecision.REMOVE_ENTRY -> { dao.deleteMirrorEntry(binding.id, path); return@forEach }
                    MirrorDecision.CONFLICT -> {
                        ...
                        if (l != null) dav.putStream(account, password, remoteHref(binding.remoteRootHref, conflictPath(path, "local-$stamp")), { requireNotNull(context.contentResolver.openInputStream(l.document.uri)) }, l.document.length().takeIf { it >= 0 }, l.document.type ?: "application/octet-stream", null, createOnly = true)
            ...
            dao.upsertMirror(binding.copy(state = if (binding.state == MirrorState.BASELINED) MirrorState.BASELINED else MirrorState.ERROR, lastError = error.message))
            throw error
        }
    }

    private suspend fun scanRemote(account: DavAccountEntity, password: CharArray, href: String, prefix: String, out: MutableMap<String, Remote>, budget: IntArray) {
    ...
    private fun hash(input: java.io.InputStream): String = MessageDigest.getInstance("SHA-256").let { digest -> input.copyTo(object : java.io.OutputStream() { override fun write(b: Int) { digest.update(b.toByte()) }; override fun write(b: ByteArray, off: Int, len: Int) { digest.update(b, off, len) } }); digest.digest().joinToString("") { "%02x".format(it) } }
    ...
    private suspend fun download(account: DavAccountEntity, password: CharArray, root: DocumentFile, path: String, href: String) {
        ...
        val temporary = parent.createFile("application/octet-stream", ".inkdav-part-${UUID.randomUUID()}") ?: error("Cannot create temporary mirror file")
        try {
            context.contentResolver.openOutputStream(temporary.uri, "wt")!!.use { output -> dav.get(account, password, href).use { it.copyTo(output) } }
    ...
    private fun ensureDirectory(root: DocumentFile, path: String): DocumentFile = path.split('/').filter(String::isNotBlank).fold(root) { current, name -> current.findFile(name)?.takeIf(DocumentFile::isDirectory) ?: current.createDirectory(name) ?: error("Cannot create local directory $name") }
    ...
    private fun remoteHref(root: String, path: String): String = path.split('/').filter(String::isNotBlank).fold(root) { current, name -> URI(current.trimEnd('/') + "/" + java.net.URLEncoder.encode(name, "UTF-8").replace("+", "%20")).toString() }
    private fun conflictPath(path: String, suffix: String): String { val dot = path.lastIndexOf('.'); return if (dot > path.lastIndexOf('/')) path.substring(0, dot) + " (InkDAV $suffix)" + path.substring(dot) else "$path (InkDAV $suffix)" }
    private fun entry(binding: String, path: String, remote: Remote?, local: Local?, old: MirrorEntryEntity?, status: MirrorEntryStatus, reason: String?) = MirrorEntryEntity(
        id = old?.id ?: UUID.nameUUIDFromBytes("$binding|$path".encodeToByteArray()).toString(), bindingId = binding, relativePath = path,
        remoteHref = remote?.value?.href ?: old?.remoteHref.orEmpty(), localDocumentUri = local?.document?.uri?.toString(), isDirectory = remote?.value?.isCollection ?: local?.document?.isDirectory ?: false,
